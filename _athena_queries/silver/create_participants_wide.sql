...
-- --------------------------
-- Explode participants (10 per match)
-- --------------------------
participants AS (
  SELECT
    m.matchid,
...
-- --------------------------
-- Participant base fields (from match payload)
-- --------------------------
p_base AS (
  SELECT
    matchid,
    ...
    COALESCE(NULLIF(json_extract_scalar(p_json,'$.individualPosition'), ''), json_extract_scalar(p_json,'$.teamPosition')) AS individual_position,
...
-- --------------------------
-- Explode timeline frames and participantFrames
-- --------------------------
frames AS (
  SELECT
    t.matchid,
...
-- --------------------------
-- Explode events (wards/items/kills/objectives)
-- --------------------------
events AS (
  SELECT
    fr.matchid,
...
-- Elite monster objective events
objective_events AS (
...
positions_ts AS (
  SELECT
    ...
    pf.participant_id,
    ARRAY_AGG(
      CAST(ROW(ts, CAST(json_extract_scalar(position_json,'$.x') AS integer), CAST(json_extract_scalar(position_json,'$.y') AS integer)) AS ROW(ts bigint, x integer, y integer))
      ORDER BY ts
    ) AS positions
...
kills_ts AS (
  ...
  FROM (
    SELECT matchid, ts, pos, killer_id, victim_id, assists_ids, killer_id AS p_id, 'KILL'  AS kind
    FROM   kill_events
    UNION ALL
    SELECT matchid, ts, pos, killer_id, victim_id, assists_ids, victim_id AS p_id, 'DEATH' AS kind
    FROM   kill_events
  ) k
...
ward_maps AS (
  SELECT
    matchid,
    participant_id,
    map_agg(ward_type, wards_placed_by_type) AS wards_placed_map
  FROM ward_events
  GROUP BY 1,2
),
ward_destroy_maps AS (
  SELECT
    matchid,
    participant_id,
    map_agg(ward_type, wards_destroyed_by_type) AS wards_destroyed_map
  FROM ward_destroys
  GROUP BY 1,2
),

-- --------------------------
-- Final select: join base + derived
-- --------------------------
final AS (
  SELECT
    b.matchid,
    ...
    CAST(ROW(b.game_start_ts, b.game_end_ts, b.patch_full, b.game_duration_sec) AS ROW(game_start_ts bigint, game_end_ts bigint, patch_full varchar, game_duration_sec integer)) AS meta,
...
SELECT
  ...
  objective_takedowns_by_type,          -- detailed keys (dragon subtypes)
  objective_takedowns_by_type_norm,     -- normalized keys
  team_objective_totals_by_type,        -- normalized keys (team totals)
  objective_participation_by_type,      -- normalized keys, fractions 0–1
