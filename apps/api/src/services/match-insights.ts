...
function buildPrompt(
  ...
  const systemPrompt = [
    ...
    '',
    'Map context:',
    "Summoner's Rift is ~15,000 units wide. Distance interpretation: <600u = very close (ability/auto-attack range), 600-1000u = close (immediate threat), 1000-2500u = nearby (can collapse quickly), >2500u = far (cross-map, ~17% of width).",
    '',
    'Events policy:',
    'Only emit keyMoments that exist in Context.events. For kills: killerId=subject => kill; victimId=subject => death; else assisted.',
    'Context.events include `participantStates` with nearest-frame positions and `proximity.participants` listing nearby champions (team + inferredPosition). Additionally, use `proximitySummary` to deterministically assess ally/enemy closeness: prefer within600 as immediate presence, within1000 as near threat, and numbersAdvantage to judge collapse risk.',
    'Positions confidence: Each event provides `frameDeltaSummary` and `positionConfidence` (0..1). If max frame delta is high (e.g., > 20s) reduce certainty about positions and avoid claims like "you were alone". Prefer analyses where `positionConfidence` is high; otherwise mention low confidence due to timeline frame lag.',
    '',
    'Naming:',
    ...
    '',
    'Quality:',
    'Be concise and actionable. Align zone/enemyHalf with coordinates when present. Do NOT include confidence scores or percentages in insight/suggestion text; use positionConfidence internally but keep reasoning clean and direct.',
    '',
    'Writing style:',
    'Write in natural, conversational English. Avoid technical codenames and jargon in player-facing text. For example, instead of "MissFortune" say "Miss Fortune" or "the enemy ADC"; instead of "NumbersAdvantage" say "outnumbered" or "2v0"; instead of "ADC-support synergy" say "coordinating with your support". Make insights feel human and accessible, not like technical logs.',
    schemaHint,
  ].join('\n');
...
export async function generateMatchInsights(
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    const command = new ConverseCommand({
      ...
      inferenceConfig: { maxTokens: 13_000, temperature: 1 },
