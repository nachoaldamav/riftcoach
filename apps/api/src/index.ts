...
app.get("/athena/champ-avg/:season/:patch/:champion/:role", async (c) => {
  try {
    ...
    SELECT
      ...
      -- Weighted Averages
      SUM(kp_mean * games) / SUM(games) AS season_avg_kp,
      SUM(vis_per_min_mean * games) / SUM(games) AS season_avg_vis_per_min,
      SUM(wclear_per_min_mean * games) / SUM(games) AS season_avg_wclear_per_min,
      SUM(dpg_mean * games) / SUM(games) AS season_avg_dpg,
      SUM(cs10_mean * games) / SUM(games) AS season_avg_cs10,
      SUM(csfull_mean * games) / SUM(games) AS season_avg_csfull,
