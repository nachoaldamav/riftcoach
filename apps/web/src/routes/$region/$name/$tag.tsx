...
export const Route = createFileRoute('/$region/$name/$tag')({
  ...
  loader: async ({ params }) => {
    ...
    try {
      ...
      const message = err instanceof HttpError ? err.message : 'Failed to load status';
      return {
        region,
        name,
        tag,
...
function RouteComponent() {
  const { region, name, tag, initialStatus, summoner, error } = Route.useLoaderData() as {
    region: string;
    name: string;
    tag: string;
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      setWsConnected(true);
      ws.send(
        JSON.stringify({ type: 'subscribe', channel: `rewind:progress:${status.rewindId}` }),
      );
    };
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-300">Level {summoner.summonerLevel}</p>
              </div>
            </div>
  ...
  const progressPercentage = status.total > 0 ? Math.min((status.processed / status.total) * 100, 100) : 0;
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-3xl mx-auto space-y-8">
          <motion.div variants={itemVariants} className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/20 border border-cyan-400/30">
              <div className={`w-2 h-2 rounded-full pulse-glow ${wsConnected ? 'bg-green-400' : 'bg-cyan-400'}`} />
              ...
              <CardBody className="p-8">
                <div className="space-y-6">
                  <div className="text-center space-y-2">
                    <h2 className="text-3xl font-bold text-white">{Math.round(progressPercentage)}%</h2>
                  ...
                  <div className="space-y-3">
                    <Progress value={progressPercentage} className="h-4" color="primary" size="lg" aria-label="Progress bar" />
              ...
              <CardBody className="p-6">
                <div className="grid grid-cols-3 gap-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-cyan-400 mb-1">{status.total}</div>
                    <div className="text-sm text-slate-300">Matches Found</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-cyan-400 mb-1">{status.processed}</div>
                    <div className="text-sm text-slate-300">Processed</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-cyan-400 mb-1">{status.position ?? '-'}</div>
            ...
            <Card className="bg-slate-600/70 backdrop-blur-sm border-slate-500 shadow-xl">
              <CardBody className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-white mb-2">Analyzing {region.toUpperCase()} Region</h3>
                    <p className="text-sm text-slate-300">Searching through Ranked queues for the complete picture</p>
                  </div>
          ...
          <motion.div variants={itemVariants} className="text-center text-muted-foreground">
            <p className="text-sm text-slate-300">Hang tight! Your personalized League of Legends rewind will be ready soon.</p>
          </motion.div>
        </motion.div>
      </div>
        ...
        <div className="fixed bottom-10 right-10 z-50">
          <div className="relative w-80">
            {processedMatches.slice(0, 6).map((match, index) => {
              ...
              const gradientClass = match.won ? 'bg-gradient-to-r from-slate-800/95 to-emerald-900/40' : 'bg-gradient-to-r from-slate-800/95 to-red-900/40';
              ...
              return (
                ...
                >
                  <Card isPressable className={`relative overflow-hidden border border-slate-600/50 shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all duration-200 ease-out rounded-medium ${gradientClass} backdrop-blur-sm`}>
                    <CardBody className="relative z-10 p-4">
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 min-w-0">
                          <ChampionImage championId={match.playerChampionId} size="md" showName={false} className="shrink-0" />
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 text-small">
                              <span className="font-medium text-slate-300">K/D/A</span>
                        ...
                        <div className="ml-auto flex items-center gap-2">
                          <ChampionImage championId={match.opponentChampionId} size="md" showName={false} className="shrink-0" />
                        </div>
                      </div>
                    </CardBody>
                  </Card>
